...
namespace ContractConfigurator.RemoteTech
{
    ...
    public class CelestialBodyCoverageRequirement : ContractRequirement
    {
        ...
        {
            string output = "Must " + (invertRequirement ? "not " : "") + "have between " + (minCoverage*100).ToString("N0") + "% and " + (maxCoverage*100).ToString("N0") + "% RemoteTech coverage of " + targetBody.theName;
